...
class Constellation(val name: String, val lines: Vector[(Star,Star)]) {
  ...
  val minCoords = {
    val x = this.stars.minBy(_.coords.x).coords.x
    val y = this.stars.minBy(_.coords.y).coords.y
    new StarCoords(x, y) 
  }
  ...
  val maxCoords = {
    val x = this.stars.maxBy(_.coords.x).coords.x
    val y = this.stars.maxBy(_.coords.y).coords.y
    new StarCoords(x, y) 
  }

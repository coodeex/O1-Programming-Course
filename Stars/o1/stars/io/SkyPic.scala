...
object SkyPic {
  ...
  def placeStar(skyPic: Pic, star: Star): Pic = {
    val kuva = circle(6.0 / (star.magnitude + 2), White)
    if (star.magnitude > - 2){
      skyPic.place(kuva, star.posIn(skyPic))
    }else{
      skyPic
    }
  }
  ...
  def placeConstellation(skyPic: o1.Pic, constellation: Constellation): o1.Pic = {
    constellation.lines.foldLeft(constellation.stars.foldLeft(skyPic)((kuva, star) => 
                  kuva.place(circle(4, Red), star.posIn(kuva))))((palloKuva, star) =>
    palloKuva.place(line(star._1.posIn(skyPic), star._2.posIn(skyPic), Yellow), star._1.posIn(skyPic)))
  }
  ...
  def create(skyData: StarMap, bgSize: Int)= {
    ...
    var kuva = skyData.stars.foldLeft(darkSky)(placeStar)
    skyData.constellations.foreach(cons => kuva = placeConstellation(kuva, cons))
    kuva
  }

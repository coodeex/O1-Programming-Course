...
object SkyFiles {
  ...
  def parseStarInfo(starAsString: String): Star = {
    val vek = starAsString.split(";").toVector
    val koordit = new StarCoords(vek(0).toDouble, vek(1).toDouble)
    val iidee = vek(3).toInt
    val koko = vek(4).toDouble
    val nimi = vek.lift(6)
    new Star(iidee, koordit, koko, nimi)

...
class TribalBot(body: RobotBody, val tribe: Tribe) extends RobotBrain(tribe.newName(), body) {
  ...
  def moveBody() = {
    if(tribe.isPacifist) {
      while(!nextInstruction.endsTurn) nextInstruction = nextInstruction.execute(this)
    }
    else{
    this.enemyInFront match{
      case Some(robo) => hack()
      case None => {
      while(!nextInstruction.endsTurn) nextInstruction = nextInstruction.execute(this)
      }
  ...
  def determineTribe(another: RobotBrain): Option[Tribe] = { //Some(this.tribe) 
    if(another.isInstanceOf[TribalBot]) Some(another.asInstanceOf[TribalBot].tribe)
    else None
  }
  ...
  def isFriend(robot: RobotBrain) = {
    this.determineTribe(robot) match{
      case Some(heimo) => heimo == this.tribe && (if(robot.isInstanceOf[TribalBot]) robot.asInstanceOf[TribalBot] != this else false)
      case None => false
    }
  }
  ...
  def talk(lineNumber: Int) = {
    if(seesFriend) this.friendInFront.get.boot(lineNumber)
  }
  ...
  def hack() = {
    if(seesEnemy){
      this.world.apply(this.body.brain.get.locationInFront).robot.get.brain = Some(new TribalBot(this.enemyInFront.get.body, this.tribe))
      this.shout(this.slotValue("hackline"))
    }
  }
  ...
  def returnFromSubprogram() = {
    ...
    this.callStack.pop.getOrElse(failWithUnderflowError).returnLine
  }


  /** Returns the brains of all the robots in the same robot world with this one. */
  def longRadar: Vector[RobotBrain] = {  //Vector[RobotBrain]()  TODO: provide a working implementation    
    this.body.world.robotList.filter(!_.brain.isEmpty).map(_.brain.get)
  }
  ...
  def directedRadar(dirNumber: Int) = {
    val dir = this.suunta(dirNumber)
    val d = this.world.neighbors(this.body.location.neighbor(dir), false).toBuffer
    d    += this.world.apply(this.body.location.neighbor(dir))
    this.shortRadar.filter(rb=> d.contains(this.world.apply(rb.location)))
  }
  
  private def suunta(numero:Int): CompassDir = {
    var n = 0
    var s = this.body.facing
    while(n != numero){
      s = s.clockwise
      n += 1
    }

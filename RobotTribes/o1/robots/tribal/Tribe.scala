...
class Tribe(val name: String, fileName: String) {
  ...
  abstract class GenericInstruction extends Instruction {
    ...
    lazy val lineNumber = {
      val index = Tribe.this.instructions.indexWhere( _ eq this )
      ...
      parameter match {
        ...
        case Left(word) =>
          if (Tribe.Slots.contains(word))
            ...
            Tribe.this.label(word).getOrElse(throw new TribeFileException(s"'$word' is not a valid line number, label, or slot name."))
      }
  ...
  class Score extends LoadInstruction(Tribe.RadarSlot) {
    def newSlotValue(actor: TribalBot) = {
      ...
      val tribeCounts = tribesPerRobot.occurrences

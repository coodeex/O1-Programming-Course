...
class GameState(val missesAllowed: Int, val previousGuesses: String, val visibleWord: String, val possibleSolutions: Vector[String])  {
  ...
  def isWon = !this.isLost && this.visibleWord.filter(_ != '_').size == this.wordLength
  ...
  private def reveal(patternToReveal: String /*, v: Vector[Boolean]*/) = {//this.visibleWord  // TODO: replace with a proper implementation and use this to implement guessLetter
    "tää on noobeille"
  ...
  def guessLetter(guess: Char) = {
    ...
    val vz = possibleSolutions.zipWithIndex// ettei hyvät indeksöinnit mene hukkaan
    val vz1 = vz.map(sz=> (sz._1.map(_==actualGuess), sz._2))
    val vz1m = vz1.groupBy(pari=> pari._1) // esim vz1m.get(Vector(false, false, false, false)) tulee vektori jolla on pituus ja jossa on pareja:
                                           // Some(Vector((Vector(false, false, false, false),4), (Vector(fal
    val vz1ms = vz1m.mapValues(_.size)     // value on se koko, tällasta : Map(Vector(false, true, true, false) -> 1, Vector(false, false, false, true) -> 2,
    val vz1mss = vz1ms.map(_.swap)         // hakurakenne toisinpäin vz1mss.get(3) tulee Some(Vector(false, false, false, false))
    //vz1mss meinaa vektori zipattu mäpätty size swäpätty
    val srk = vz1ms.values.max             // suurimman ryhmän koko
    val srh = vz1m(vz1mss.get(srk).get) //OrElse(scala.collection.immutable.IndexedSeq[Boolean]()))  ettii suurimman sanaryhmän kokoelman
    val sri = srh.map(a => a._2)           // suurimman sanaryhmän indeksit
    val sr = Buffer[String]()              // suurin ryhmä
    for(nro<-sri){sr += possibleSolutions(nro)}// hoitaa hommat
    
    val b2 = vz1mss.get(srk).get.zipWithIndex.filter(pari => pari._1).map(pari=>pari._2) // ettii missä kohissa on true
    val p = b2.toList.foldLeft(visibleWord)((s, i) => s.updated(i, actualGuess))         // tekee taikatempun
    var v = if(visibleWord == p){1}else{0}
    
    new GameState(this.missesAllowed - v, this.previousGuesses + actualGuess, p, sr.toVector)
    
    /*val solution = this.possibleSolutions.head
    val howMany = this.previousGuesses.length + 1
    val oneMoreRevealed = solution.take(howMany) + this.visibleWord.drop(howMany)
    new GameState(this.missesAllowed, this.previousGuesses + actualGuess, oneMoreRevealed, Vector(solution))*/
  }

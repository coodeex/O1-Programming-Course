...
package object o1 extends o1.util.ShortcutAliases {
  ...
  def metreiksi(jalat: Double, tuumat: Double) = (jalat * 12 + tuumat) * 2.54 / 100

  def rivi(ruutu: Int) = ruutu / 8

  def sarake(ruutu: Int) = ruutu % 8 

  def saesta(teksti: String, melodia: String) = {
	  println(teksti)
	  play(melodia)
  }
  
  def pystypalkki(leveys: Int) = {
	  val korkeus = leveys * 10
	  rectangle(leveys, korkeus, Blue)
  }
  

  def pystypalkki(leveys: Int, vari: Color) = {
	  val korkeus = leveys * 10
	  rectangle(leveys, korkeus, vari)
  }

  def kurssiarvosana(teh: Int, tentti: Int, aktiivi: Int) = {
    val summa = teh + tentti + aktiivi 
    val arvosana = min(summa, 5)
    arvosana
  }

  def liigapisteet(voitto: Int, tasa: Int) = {
    val liigapisteet = voitto * 3 + tasa
    liigapisteet
  }
  
  def joukkueenTiedot(nimi: String, voitto: Int, tasa: Int, tappio: Int) = {
    val pelienMaara = voitto + tasa + tappio
    val palautus = nimi + ": " + voitto + "/" + pelienMaara + " voittoa, " + tasa + "/" + pelienMaara + " tasapeliä, " + tappio + "/" + pelienMaara + " tappiota, " + liigapisteet(voitto, tasa) + " pistettä"
    palautus
  }
  
  def tuplaaPisteet(puskuri: Buffer[Int], pelaaja: Int) = {
    puskuri(pelaaja - 1) = puskuri(pelaaja - 1) * 2
  }
  
  def sakko(puskuri: Buffer[Int], pelaaja: Int, vah: Int) = {
    val vahennetaan = min(puskuri(pelaaja - 1) - 1, vah)
    puskuri(pelaaja - 1) = puskuri(pelaaja - 1) - vahennetaan
    vahennetaan
  }
  ...
  def somalianLippu(leveys: Double) = {
    ...
    val pohja = rectangle(leveys, korkeus, RoyalBlue)
    val tahti = o1.star(tahdenLeveys, White)
    tahti.onto(pohja)
  }
  
  def suomenLippu(leveys: Double) = {
    val x = leveys / 18.0
    val vas = rectangle(5 * x, 11 * x, White)
    val siniPysty = rectangle(3 * x, 11 * x, Blue)
    val oik = rectangle(10 * x, 11 * x, White)
    val melkeenValmis = vas.leftOf(siniPysty.leftOf(oik))
    val siniVaaka = rectangle(leveys, 3 * x, Blue)
    siniVaaka.onto(melkeenValmis)
  }
...
//    kurssiarvosana(tehtavaarvosana, tenttibonus, aktiivisuusbonus)
//    kuvaukset(kurssiarvosana)

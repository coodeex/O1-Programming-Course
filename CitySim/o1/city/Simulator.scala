...
class Simulator {
  ...
  def findDemographic(demographic: Demographic): Vector[GridPos] = { // TODO: implementation missing
    this.allAddresses.filter(i => this.cityMap.matches(i, demographic))
  }
  ...
  def dissatisfiedResidents: Vector[GridPos] = {  
      def dissatisfied(sijainti: GridPos) = {
      val naapurit = this.cityMap.neighbors(sijainti, true)
      val samanlaisia = naapurit.filter( _ == this.cityMap.elementAt(sijainti))
      val lukum = naapurit.filter( _.isInstanceOf[Occupied]).size
      val q = (samanlaisia.size / lukum.toDouble) * 100
      this.similarityDesired > q
    }
    allAddresses.filter(dissatisfied(_)).filter(this.cityMap.elementAt(_).isInstanceOf[Occupied])
  }
  /** Advances the most recently launched simulation by one step, moving all dissatisfied
    ...
    *      muuttajat.map((i => val e = vacantAddresses(Random.nextInt(vacantAddresses.size))))
  ...
  def moveResidents(): Unit = {
    ...
    val muuttajat = Random.shuffle(this.dissatisfiedResidents).toBuffer
    
    if (vacantAddresses.nonEmpty) {
      muuttajat.map(osote => this.cityMap.swap(osote,this.findDemographic(Vacant).toBuffer(Random.nextInt(this.findDemographic(Vacant).toBuffer.size))))
    }
  }
  ...
  def residents: Map[Demographic, Vector[GridPos]] = {
    this.cityMap.allPositions.groupBy(this.cityMap.apply(_))
  }
  ...
  /** Returns the proportion of satisfied residents in the city. For example, a return value
    ...
    * @see [[dissatisfiedResidents]] */
  def satisfactionLevel = 1 - this.dissatisfiedResidents.size.toDouble / this.cityMap.size

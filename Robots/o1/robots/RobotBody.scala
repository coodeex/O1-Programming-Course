...
class RobotBody(val world: RobotWorld, initialLocation: GridPos, initialFacing: CompassDir) {
  ...
  def neighboringSquare(direction: CompassDir) = {
    this.world.elementAt(this.coordinates.neighbor(direction))
  }
  ...
  def canMoveTowards(direction: CompassDir) = {
    (this.neighboringSquare(direction)).isInstanceOf[Floor]
  }
  ...
  /** Determines whether the robot is stuck or not, that is, if there are any squares around
    ...
    * @see [[canMoveTowards]] */
  def isStuck = {
    this.world.neighbors(this.coordinates, false).forall(i=>(!i.isInstanceOf[Floor]))
  }

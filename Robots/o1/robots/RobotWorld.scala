...
class RobotWorld(floorWidth: Int, floorHeight: Int) extends Grid[Square](floorWidth + 2, floorHeight + 2) { // TODO: add "extends ..."
  ...
  def addRobot(initialLocation: GridPos, initialFacing: CompassDir): RobotBody = {
    val uusi = new RobotBody(this, initialLocation, initialFacing)
    this.robots = this.robots :+ uusi
    this.elementAt(initialLocation).addRobot(uusi)
    
   // if(this.turn == this.robots.size) this.turn == 0
    uusi
  ...
  /** Returns the robot whose turn it is to act. That is, returns the robot who will be the
    ...
    *if(Some(this.robots.size).exists( _ > 0) && this.turn == 0){
      this.robots.lift((this.turn - 1) % this.robots.size)
    }else 
  ...
  def nextRobot: Option[RobotBody] = {
    if (Some(this.robots.size).exists( _ > 0)){
      this.robots.lift(this.turn)
    }else{None}
  }
  ...
  /** Causes all the robots in the world to take a turn, starting with the one whose turn it is next.
    ...
    * @see [[RobotBody.takeTurn]] */
  def advanceFullTurn() = {
    this.robots.foreach( i => advanceTurn())
  }

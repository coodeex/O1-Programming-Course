...
abstract class RobotBrain(initialName: String, val body: RobotBody) {
  ...
  def squareInFront = this.body.neighboringSquare(this.facing)
  ...
  def robotInFront = {
    this.world.elementAt(this.locationInFront).robot match{
      case Some(jotai) => jotai.brain
      case None => None
    }
  }
  ...
  def moveCarefully(): Boolean = {
    if(this.squareInFront.isInstanceOf[Floor] && this.squareInFront.isEmpty){

...
class GameBoard(width: Int, height: Int, boozeCount: Int) extends Grid[Glass](width, height) {
  ...
  private def placeBoozeAtRandom(howMany: Int) = {
    Random.shuffle(this.allElements).take(howMany).foreach( _.pourBooze())
  }
  ...
  def isOutOfWater: Boolean = {
    this.allElements.forall( lasi => lasi.isEmpty || lasi.isBooze)
  }
  ...
  def drink(target: Glass): Unit = {
    if (target.isBooze) {
      this.boozeGlasses.map( lasi => lasi.empty())
    } else if(target.dangerLevel == 0) {
      target.empty()
      
      
      val a = target.neighbors
      val b = a.filter(a=> !a.isEmpty)
      b.foreach(_.empty)
      
      
      
      b.foreach(drink(_))
      //target.neighbors.filter(_ != target).foreach(t => drink(t))
    }
    else target.empty()

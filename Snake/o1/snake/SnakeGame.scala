...
class SnakeGame(initialPos: GridPos, initialHeading: CompassDir) {
  ...
  def isOver = {
    ...
    val collidedWithItself = segments.tail.contains(head)
  ...
  def advance() = {
    if(nextFood == segments.head.neighbor(snakeHeading)){
      nextFood = randomLocationOnGrid()
      this.segments = this.segments.head.neighbor(snakeHeading) +: segments
    }else{
      this.segments = this.segments.head.neighbor(snakeHeading) +: this.segments.init
    }
  }
